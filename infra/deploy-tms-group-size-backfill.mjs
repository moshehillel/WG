...
const outDir = path.join(__dirname, 'tms-api-group-size-backfill-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-group-size-backfill.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-group-size-backfill-deploy-out.txt'),
  [
    'TMS Small Group null groupSize → backfill 2 on load; Related Service group parse; FE label Kind→type',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
