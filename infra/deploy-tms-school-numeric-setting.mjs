...
const outDir = path.join(__dirname, 'tms-api-school-numeric-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-school-numeric.zip');
...
if (!bundled.includes('does not match child') || !bundled.includes('union free')) {
  throw new Error('Bundle missing school-match helpers — aborting deploy');
}
console.log('bundle guard: school match OK');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-school-numeric-deploy-out.txt'),
  [
    'TMS Frontline: ignore numeric/ratio setting tokens (school=1 regression)',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
...
console.log('wrote cdk-tms-school-numeric-deploy-out.txt');
