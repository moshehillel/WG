...
const outDir = path.join(__dirname, 'tms-api-session-notes-report-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-session-notes-report.zip');
...
if (!reportsDist.includes('sessionNotesReport')) {
  throw new Error(
...
if (!bundled.includes('sessionNotesReport') && !bundled.includes('/admin/reports/session-notes')) {
  throw new Error('Bundle missing session-notes report — aborting deploy');
}
if (!bundled.includes('session-notes')) {
  throw new Error('Bundle missing session-notes path string — aborting deploy');
}
console.log('bundle guard: session-notes report + solo-group mandate bridge OK');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-session-notes-report-deploy-out.txt'),
  [
    'TMS admin Session notes report (attended vs missed by dateOfService)',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
