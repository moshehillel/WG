...
const outDir = path.join(__dirname, 'tms-api-hha-contract-74-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-hha-contract-74.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-hha-contract-74-deploy-out.txt'),
  [
    'TMS HHA ErrorID=-74: AddPatientContract from student.programType before CreateSchedule',
    `Function: ${fnName}`,
    `GitSHA: ${sha}`,
    out.trim(),
    env.trim(),
