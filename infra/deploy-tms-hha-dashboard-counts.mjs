/**
 * Deploy: Admin HHA dashboard — confirmed X of Y eligible; week status from transfer rollup.
 */
...
const outDir = path.join(__dirname, 'tms-api-hha-dashboard-counts-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-hha-dashboard-counts.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-hha-dashboard-counts-deploy-out.txt'),
  [
    'HHA dashboard counts — confirmed of eligible; week rollup status',
    `Function: ${fnName}`,
    out.trim(),
    '',
  ].join('\n'),
