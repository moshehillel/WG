...
const outDir = path.join(__dirname, 'tms-api-calendar-fallback-warn-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-calendar-fallback-warn.zip');
...
console.log(
  [
    'TMS calendar Mon–Fri fallback warning',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Empty school calendar → explicit warning on week/import/save/submit; Mon–Fri still computed',
    '',
  ].join('\n'),
);
