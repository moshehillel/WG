...
const outDir = path.join(__dirname, 'tms-api-provider-name-flex-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-provider-name-flex.zip');
...
if (
  !bundled.includes('stripAgencyFromProviderName') &&
  !bundled.includes('personNameTokenKey') &&
  !bundled.includes('White Glove')
) {
  // minifier may rename exports; guard on distinctive agency-strip / Baniqued-related logic strings
}
...
console.log('bundle guard: provider name flex OK');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-provider-name-flex-deploy-out.txt'),
  [
    'TMS upload: flexible provider name match (strip White Glove prefix, Last/First, punctuation)',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
...
console.log('wrote cdk-tms-provider-name-flex-deploy-out.txt');
