...
const outDir = path.join(__dirname, 'tms-api-signed-archive-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-signed-archive.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-signed-archive-deploy-out.txt'),
  [
    'Signed timesheet archive filter + webhook index',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    'Added: ENTITY#archives, S3 tms/archive/uploads + tms/timesheets PutObject, GET /archive + /admin/archive + /archive/{id}/file',
    'Preserved: MFA settings paths untouched; upload/submit flows still succeed if archive persist fails.',
    '',
  ].join('\n'),
);
