...
const outDir = path.join(__dirname, 'tms-api-mandate-billing-name-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-mandate-billing-name.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-mandate-billing-name-deploy-out.txt'),
  [
    'TMS mandate billingServiceName at caseload import + HHA transfer prefer stored',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Wired: Mandate.billingServiceName from discipline+RS Duration (PT school 30)',
    'Transfer prefers stored name; Related Service (serviceType) unchanged for therapists',
    '',
  ].join('\n'),
);
