...
const outDir = path.join(__dirname, 'tms-api-caseload-dob-school-setup-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-caseload-dob-school-setup.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-caseload-dob-school-setup-deploy-out.txt'),
  [
    'TMS final WG caseload DOB + school incomplete (calendar+address)',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Wired: Student BirthDate → student.dob → CreatePatient; Min → duration; Provider alias',
    'School setup: incomplete until calendar AND address (API setupBySchoolId + admin detail flags)',
    '',
  ].join('\n'),
);
