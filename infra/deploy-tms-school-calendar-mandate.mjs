...
const outDir = path.join(__dirname, 'tms-api-school-calendar-mandate-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-school-calendar-mandate.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-school-calendar-mandate-deploy-out.txt'),
  [
    'TMS school calendar → live mandate over-checks',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Wired: calendarByStudentId into week GET, upload, session save, week submit',
    'Cycle windows: Mon–Fri minus off days; weekends never count; densest N hard-block',
    '',
  ].join('\n'),
);
