...
const outDir = path.join(__dirname, 'tms-api-createschedule-date-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-createschedule-date.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-createschedule-date-deploy-out.txt'),
  [
    'TMS CreateSchedule VisitDate MM/DD/YYYY → YYYY-MM-DD (AllXsd)',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Fix: buildCreateScheduleBody uses psDateToIso so VisitDate is xs:date',
    '',
  ].join('\n'),
);
