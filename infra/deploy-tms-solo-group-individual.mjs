...
const outDir = path.join(__dirname, 'tms-api-solo-group-individual-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-solo-group-individual.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-solo-group-individual-deploy-out.txt'),
  [
    'TMS solo-group → individual rate + group clarifications',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    'Base: prior overlap CodeSha256 F7AWW3QvkNhNM2rwvMr+NDmhvXF+EtlqZk/oidXs038=',
    '',
    'Rules:',
    '1. Group-tagged with ≥1 present (attended/makeup) peer → group 30/42/45 (or hourly) + OT Group $rate',
    '2. Group-tagged solo (peers absent/missed) → individual rate + OT $rate (timesheet + HHA)',
    '3. Group-mandate seen individually (no group token) → individual pay; note should say no peer available; individual for overlap (later peer blocked)',
    '4. group↔group share window only when both sessions are group-tagged',
    '5. Fewer than mandate groupSize OK; more than groupSize blocked',
    '6. Small Group with null size → cap 2 (fewer than 3)',
    '',
  ].join('\n'),
);
