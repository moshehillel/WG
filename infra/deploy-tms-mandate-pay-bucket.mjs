...
const outDir = path.join(__dirname, 'tms-api-mandate-pay-bucket-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-mandate-pay-bucket.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-mandate-pay-bucket-deploy-out.txt'),
  [
    'TMS pay/billing duration from mandate (not Frontline nearest)',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    ...
    '',
    'Rules:',
    '1. School 30/42/45 pay bucket from matching mandate durationMinutes',
    '2. Do not round Frontline begin/end to nearest pay bucket',
    '3. Example: 40-min clock + 30-min mandate → 30-min rate (not 42)',
    '4. Solo group → individual rate; peers present → group rate (still mandate duration)',
    '5. HHA school billing service code uses same mandate duration',
    '',
  ].join('\n'),
);
