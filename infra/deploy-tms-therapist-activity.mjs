...
const outDir = path.join(__dirname, 'tms-api-therapist-activity-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-therapist-activity.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-therapist-activity-deploy-out.txt'),
  [
    'Therapist Activity PDF import',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    'Preserved: all-or-nothing import, Eval pay, school picker, overlap, DocuSign, Dynamo.',
    '',
  ].join('\n'),
);
