/**
 * Deploy: SignNow webhook locks/archives/emails only — no HHA transfer on sign.
 * HHA remains Wednesday auto-transfer + admin Send to HHA.
 * Rebuild shared + tms-db, then bundle TmsApiFn.
 */
...
const outDir = path.join(__dirname, 'tms-api-esign-no-hha-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-esign-no-hha.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-esign-no-hha-deploy-out.txt'),
  [
    'TMS SignNow webhook: lock + signed PDF + archive + therapist email — NO HHA on sign',
    'HHA only: Wed auto-transfer (WhiteGlove-TmsHhaAutoTransfer) + admin POST /weeks/:id/hha',
    `Function: ${fnName}`,
    out.trim(),
    '',
  ].join('\n'),
);
console.log('wrote cdk-tms-esign-no-hha-deploy-out.txt');
