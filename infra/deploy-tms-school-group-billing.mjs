...
const outDir = path.join(__dirname, 'tms-api-school-group-billing-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-school-group-billing.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-school-group-billing-deploy-out.txt'),
  [
    'TMS group mandate billingServiceName → {Disc} school group {bucket}',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Before: group mandate → PT school 30',
    'After:  group mandate → PT school group 30 (OT/SLP same pattern)',
    'Individual stays PT school 30; load backfill rewrites wrong group stamps',
    '',
  ].join('\n'),
);
