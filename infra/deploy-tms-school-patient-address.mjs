...
const outDir = path.join(__dirname, 'tms-api-school-patient-address-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-school-patient-address.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-school-patient-address-deploy-out.txt'),
  [
    'TMS HHA CreatePatient uses school address',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Wired: School.address1/city/state/zipCode → upsertPatient on CreatePatient last resort',
    'DOB: still student.dob only — waiting on Moshe caseload sample (no fake DOB)',
    '',
  ].join('\n'),
);
