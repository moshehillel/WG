/**
 * Deploy: HHA caregiver lookup — strip PT-star / OT / SLP discipline suffixes before SearchCaregivers.
 */
...
const outDir = path.join(__dirname, 'tms-api-hha-discipline-suffix-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-hha-discipline-suffix.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-hha-discipline-suffix-deploy-out.txt'),
  [
    'HHA caregiver lookup — strip PT-star / OT / SLP discipline suffixes before SearchCaregivers',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    '',
