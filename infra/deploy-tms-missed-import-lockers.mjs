...
const outDir = path.join(__dirname, 'tms-api-missed-import-lockers-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-missed-import-lockers.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-missed-import-lockers-deploy-out.txt'),
  [
    'Missed-session import lockers — status detection, duplicate key + attendance, skip CPT/sig/note for missed',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    ...
    '',
    'Rules:',
    '1. Missed stays missed (Frontline + Therapist Activity absence detection)',
    '2. Exact duplicate = child + DOS + begin + end + attendance (missed ≠ attended)',
    '3. Missed: no CPT, signature, or service-note requirement',
    '',
  ].join('\n'),
);
