...
const outDir = path.join(__dirname, 'tms-api-cognito-mfa-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-cognito-mfa.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-cognito-mfa-deploy-out.txt'),
  [
    'TMS Cognito MFA settings (requireMfa / allowSmsMfa on /admin/settings + /me)',
    `Function: ${fnName}`,
    out.trim(),
    'Pool MFA OPTIONAL + device tracking deployed via CDK separately',
    '',
  ].join('\n'),
);
