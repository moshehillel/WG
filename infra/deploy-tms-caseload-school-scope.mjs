...
const outDir = path.join(__dirname, 'tms-api-caseload-school-scope-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-caseload-school-scope.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-caseload-school-scope-deploy-out.txt'),
  [
    'TMS therapist school picker scoped to caseload schoolIds only',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'API: schoolsForProvider = unique student.schoolId from mandates where providerId matches',
    'FE: 0 schools → empty state; 1 school → auto-select skip picker; 2+ → picker only those schools',
    'Admin /admin/schools unchanged',
    '',
  ].join('\n'),
);
