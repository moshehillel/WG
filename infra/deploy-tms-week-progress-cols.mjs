...
const outDir = path.join(__dirname, 'tms-api-week-progress-cols-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-week-progress-cols.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-week-progress-cols-deploy-out.txt'),
  [
    'TMS weekly session progress: provider name + program type',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    ...
    '',
    'Changes:',
    '1. weekProgressReport includes programType from student',
    '2. providerName already from mandate provider',
    '3. week-progress.xlsx adds Provider + Program type columns',
    '4. FE: app.js?v=91',
    '',
  ].join('\n'),
);
