...
const outDir = path.join(__dirname, 'tms-api-luna-handoff-contact-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-luna-handoff-contact.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-luna-handoff-contact-deploy-out.txt'),
  [
    'TMS Luna handoff requires contact name + email',
    `Function: ${fnName}`,
    out.trim(),
    'API: POST /support/luna/handoff body.contactName + body.contactEmail (required)',
    'Email: CONTACT block at top; subject uses contact name/email',
    '',
  ].join('\n'),
);
