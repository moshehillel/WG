...
const outDir = path.join(__dirname, 'tms-api-westbury-powells-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-westbury-powells.zip');
...
if (!bundled.includes('union free') || !bundled.includes('does not match child')) {
  throw new Error('Bundle missing district/building school match — aborting deploy');
}
console.log('bundle guard: westbury/powells school match OK');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-westbury-powells-deploy-out.txt'),
  [
    'TMS Frontline: Powells Lane setting + Westbury UFSD district header match',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
...
console.log('wrote cdk-tms-westbury-powells-deploy-out.txt');
