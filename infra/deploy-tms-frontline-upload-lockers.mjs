...
const outDir = path.join(__dirname, 'tms-api-frontline-upload-lockers-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-frontline-upload-lockers.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-frontline-upload-lockers-deploy-out.txt'),
  [
    'Frontline upload lockers — multi-CPT merge, missed no CPT/times, missed reason required, copy-paste excludes missed',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    ...
    '',
    'Rules:',
    '1. Missed: no CPT required',
    '2. Missed: no begin/end times (do not invent from later rows)',
    '3. Frontline multi-CPT split rows (same child+clock, different CPT) merge to one session',
    '4. Copy-paste: missed notes are not sources for attended blocks',
    '5. Missed: require recognizable reason (e.g. Provider Absence)',
    '',
  ].join('\n'),
);
