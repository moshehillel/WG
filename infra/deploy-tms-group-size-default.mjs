...
const outDir = path.join(__dirname, 'tms-api-group-size-default-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-group-size-default.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-group-size-default-deploy-out.txt'),
  [
    'TMS Group/Small Group Ratio → groupSize default 2',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Wired: parseGroupSize Group/Small Group → 2; create/edit mandate defaults; FE shows 2 for legacy null',
    '',
  ].join('\n'),
);
