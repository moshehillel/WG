...
const outDir = path.join(__dirname, 'tms-api-madison-admin-sessions-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-madison-admin-sessions.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-madison-admin-sessions-deploy-out.txt'),
  [
    'Madison: admin session visibility, Carle Place school aliases, yellow warnings show both toggles',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    '',
