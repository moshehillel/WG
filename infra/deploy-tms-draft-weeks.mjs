...
const outDir = path.join(__dirname, 'tms-api-draft-weeks-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-draft-weeks.zip');
...
if (!bundled.includes('draftWeeks') || !bundled.includes('draftSessions')) {
  throw new Error('Bundle missing draftWeeks/draftSessions — aborting deploy');
}
console.log('bundle guard: draftWeeks + draftSessions OK');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-draft-weeks-deploy-out.txt'),
  [
    'TMS provider Draft tab: GET /weeks returns draftWeeks + draftSessions',
    `Function: ${fnName}`,
    ...
    '',
    'Changes:',
    '1. GET /weeks includes draftWeeks (draft/reopened with sessions) and draftSessions',
    '2. Week rows include draft flag, programType, schoolId, signer fields',
    '3. FE Draft tab (app.js?v=106) lists prior draft weeks + send timesheet',
  ].join('\n') + '\n',
);
console.log('wrote cdk-tms-draft-weeks-deploy-out.txt');
