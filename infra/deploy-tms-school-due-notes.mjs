...
const outDir = path.join(__dirname, 'tms-api-school-due-notes-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-school-due-notes.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-school-due-notes-deploy-out.txt'),
  [
    'TMS school report due assignments: Type + Due Date + Notes',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Wired: DueDate.notes; POST /admin/due-dates accepts notes + multiple assignments; UI Type/Due Date/Notes',
    '',
  ].join('\n'),
);
