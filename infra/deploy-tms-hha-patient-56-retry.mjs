...
const outDir = path.join(__dirname, 'tms-api-hha-patient-56-retry-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-hha-patient-56-retry.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-hha-patient-56-retry-deploy-out.txt'),
  [
    'TMS HHA ErrorID=-56: clear bad hhaPatientId → search → CreatePatient only if missing → save → retry CreateSchedule',
    `Function: ${fnName}`,
    `GitSHA: ${sha}`,
    out.trim(),
    env.trim(),
