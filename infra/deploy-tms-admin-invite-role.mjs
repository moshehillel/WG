...
const outDir = path.join(__dirname, 'tms-api-admin-invite-role-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-admin-invite-role.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-admin-invite-role-deploy-out.txt'),
  [
    'TMS admin invite role fix: Cognito group sync on UsernameExists; Dynamo admin preserved; /me hides provider for admins',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    'Billu Markowitz: Cognito Admin group + removed Therapist; Dynamo already admin',
    '',
  ].join('\n'),
);
