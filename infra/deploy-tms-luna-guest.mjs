...
const outDir = path.join(__dirname, 'tms-api-luna-guest-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-luna-guest.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-luna-guest-deploy-out.txt'),
  [
    'TMS Luna available before login (guest + rate limits)',
    `Function: ${fnName}`,
    out.trim(),
    'API: POST /support/luna/chat and /support/luna/handoff work without Cognito',
    'Guest rate limits: chat 30/15m, handoff 5/hour per X-Forwarded-For',
    '',
  ].join('\n'),
);
