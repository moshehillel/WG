...
/** ECR + CodeBuild to build the Playwright bot image entirely in AWS (no local Docker). */
export class ProviderSoftBotImage extends Construct {
  ...
  constructor(scope: Construct, id: string, props: { repoRoot: string }) {
    ...
    this.repository = new ecr.Repository(this, 'Repository', {
      repositoryName: 'white-glove/providersoft-bot',
      removalPolicy: cdk.RemovalPolicy.RETAIN,
      emptyOnDelete: false,
      lifecycleRules: [{ maxImageCount: 5, description: 'Keep last 5 bot images' }],
    });
    ...
    this.project = new codebuild.Project(this, 'BuildProject', {
      projectName: 'white-glove-providersoft-bot',
      role: buildRole,
      description: 'Build ProviderSoft Playwright Lambda container (runs in AWS, not on developer laptop)',
      ...
      buildSpec: codebuild.BuildSpec.fromAsset(
        path.join(props.repoRoot, 'infra/codebuild/buildspec-providersoft-bot.yml'),
      ),
