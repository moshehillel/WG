...
const outDir = path.join(__dirname, 'tms-api-luna-handoff-copy-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-luna-handoff-copy.zip');
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-luna-handoff-copy-deploy-out.txt'),
  [
    'TMS Luna handoff confirmation uses contact email only',
    `Function: ${fnName}`,
    out.trim(),
    env.trim(),
    'HHA: sandbox preserved (USE_MOCK=false, USE_PRODUCTION=false, ALLOW_PRODUCTION=false)',
    '',
    'Reply: Thanks — I’ve sent this to our team. We’ll follow up with you at {herEmail}.',
    'API response no longer includes internal support `to` address.',
    '',
  ].join('\n'),
);
