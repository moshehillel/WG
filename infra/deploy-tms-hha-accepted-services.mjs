/**
 * Deploy TMS API: CreatePatient AcceptedServices from PT/OT/ST discipline
 * + clearer CreateSchedule -310 error text (shift overlap / OT-only mismatch).
 */
...
const outDir = path.join(__dirname, 'tms-api-hha-accepted-services-asset');
const outfile = path.join(outDir, 'index.mjs');
const zipPath = path.join(__dirname, 'tms-api-hha-accepted-services.zip');
...
try {
  ...
  console.warn('hha-client full tsc failed; continuing with existing dist');
}
try {
  execSync('npm run build -w @white-glove/shared', { stdio: 'inherit', cwd: repoRoot });
} catch {
  console.warn('shared full tsc failed; continuing');
}
try {
  execSync('npm run build -w @white-glove/tms-api', { stdio: 'inherit', cwd: repoRoot });
} catch {
  console.warn('tms-api full tsc failed; esbuild will bundle from src');
}
...
fs.writeFileSync(
  path.join(__dirname, 'cdk-tms-hha-accepted-services-deploy-out.txt'),
  [
    'TMS HHA AcceptedServices from session/provider discipline (no silent OT)',
    '+ clearer CreateSchedule -310 text (shift overlap / OT-only mismatch)',
    `Function: ${fnName}`,
    `GitSHA: ${sha}`,
    out.trim(),
    env.trim(),
    'HHA prod preserved (USE_MOCK=false, USE_PRODUCTION=true, ALLOW_PRODUCTION=true)',
    '',
  ].join('\n'),
);
console.log('wrote cdk-tms-hha-accepted-services-deploy-out.txt');
